...
void Main_Page_six::re_test_os_slots()
{
    ...
    arguments<<g_sWorkingPath + "/Scripts/otherinfo/systeminfo.sh";
...
void Main_Page_six::re_test_hard_slots()
{
    ...
    QString program = "bash";
    QStringList arguments;
    arguments<<g_sWorkingPath + "/Scripts/otherinfo/hardwareinfo.sh";
    pro->start(program, arguments);
    while(pro->waitForReadyRead())
    {
        out += pro->readAll();
...
void Main_Page_six::more_detail_hard_slots()
{
    ...
    else if(Flag_Read_Pwd == 1)
    {
        QString Program = "bash";
        QStringList arguments;
        arguments<<g_sWorkingPath + "/Scripts/otherinfo/hardwareinfomore.sh"<<PWD;
        pro->start(Program, arguments);
...
void Main_Page_six::re_test_os_run_slots()
{
    ...
    QString program = "bash";
    QStringList arguments;
    arguments<<g_sWorkingPath + "/Scripts/otherinfo/systemrun.sh";
    pro->start(program, arguments);
    while(pro->waitForReadyRead())
    {
        out += pro->readAll();
...
void Main_Page_six::more_detail_os_run_slots()
{
    ...
    else if(Flag_Read_Pwd == 1)
    {
        QString Program = "bash";
        QStringList arguments;
        arguments<<g_sWorkingPath + "/Scripts/otherinfo/startup.sh"<<PWD;
        pro->start(Program, arguments);
        while(pro->waitForFinished())
        {
            out += pro->readAll();
